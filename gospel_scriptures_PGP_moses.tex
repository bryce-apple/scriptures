% ----------------------
\subsection{Moses}
% ----------------------
	\label{MOSES}

	% -----
	\subsubsection{Moses 1} % *MOSES-1
	% -----
		\label{MOSES-1}
		
		% --
		\paragraph{Moses 1:1} % *MOSES-1-1
		% ---
			\label{MOSES-1-1}

			THE words of God, which he spake unto Moses at a time when Moses was caught up into an exceedingly high mountain,
			
			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
			
		% --
		\paragraph{Moses 1:2} % *MOSES-1-2
		% ---
			\label{MOSES-1-2}

			And he saw God face to face, and he talked with him, and the glory of God was upon Moses; therefore Moses could endure his presence.
			
			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
			
		% --
		\paragraph{Moses 1:3} % *MOSES-1-3
		% ---
			\label{MOSES-1-3}

			And God spake unto Moses, saying: Behold, I am the Lord God Almighty, and Endless is my name; for I am without beginning of days or end of years; and is not this endless?%
				\footnote{%
					\label{MOSES-1-3-NOTE-endless}%
					See \hyperref[DC19-4]{DC 19:4} and the verses that follow it; the ``endless'' language comes from there, and that section may have a further influence on this chapter as well.
					}
					
			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
					
		% ---
		\paragraph{Moses 1:4} % *MOSES-1-4 
		% ---
			\label{MOSES-1-4}

			And, behold, thou art my son; wherefore look, and I will show thee the workmanship of mine hands; but not all, for my works are without end, and also my words, for they never cease.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:5} % *MOSES-1-5 
		% ---
			\label{MOSES-1-5}

			Wherefore, no man can behold all my works, except he behold all my glory; and no man can behold all my glory, and afterwards remain in the flesh on the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:6} % *MOSES-1-6 
		% ---
			\label{MOSES-1-6}

			And I have a work for thee, Moses, my son; and thou art in the similitude of mine Only Begotten; and mine Only Begotten is and shall be the Savior, for he is full of grace and truth; but there is no God beside me, and all things are present with me, for I know them all.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:7} % *MOSES-1-7 
		% ---
			\label{MOSES-1-7}

			And now, behold, this one thing I show unto thee, Moses, my son, for thou art in the world, and now I show it unto thee.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:8} % *MOSES-1-8 
		% ---
			\label{MOSES-1-8}

			And it came to pass that Moses looked, and beheld the world upon which he was created; and Moses beheld the world and the ends thereof, and all the children of men which are, and which were created; of the same he greatly marveled and wondered.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:9} % *MOSES-1-9 
		% ---
			\label{MOSES-1-9}

			And the presence of God withdrew from Moses, that his glory was not upon Moses; and Moses was left unto himself. And as he was left unto himself, he fell unto the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:10} % *MOSES-1-10 
		% ---
			\label{MOSES-1-10}

			And it came to pass that it was for the space of many hours before Moses did again receive his natural strength like unto man; and he said unto himself: Now, for this cause I know that man is nothing, which thing I never had supposed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:11} % *MOSES-1-11 
		% ---
			\label{MOSES-1-11}

			But now mine own eyes have beheld God; but not my natural, but my spiritual eyes, for my natural eyes could not have beheld; for I should have withered and died in his presence; but his glory was upon me; and I beheld his face, for I was transfigured before him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:12} % *MOSES-1-12 
		% ---
			\label{MOSES-1-12}

			And it came to pass that when Moses had said these words, behold, Satan came tempting him, saying: Moses, son of man, worship me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:13} % *MOSES-1-13 
		% ---
			\label{MOSES-1-13}

			And it came to pass that Moses looked upon Satan and said: Who art thou? For behold, I am a son of God, in the similitude of his Only Begotten; and where is thy glory, that I should worship thee?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:14} % *MOSES-1-14 
		% ---
			\label{MOSES-1-14}

			For behold, I could not look upon God, except his glory should come upon me, and I were transfigured before him. But I can look upon thee in the natural man. Is it not so, surely?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:15} % *MOSES-1-15 
		% ---
			\label{MOSES-1-15}

			Blessed be the name of my God, for his Spirit hath not altogether withdrawn from me, or else where is thy glory, for it is darkness unto me? And I can judge between thee and God; for God said unto me: Worship God, for him only shalt thou serve.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:16} % *MOSES-1-16 
		% ---
			\label{MOSES-1-16}

			Get thee hence, Satan; deceive me not; for God said unto me: Thou art after the similitude of mine Only Begotten.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:17} % *MOSES-1-17 
		% ---
			\label{MOSES-1-17}

			And he also gave me commandments when he called unto me out of the burning bush, saying: Call upon God in the name of mine Only Begotten, and worship me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:18} % *MOSES-1-18 
		% ---
			\label{MOSES-1-18}

			And again Moses said: I will not cease to call upon God, I have other things to inquire of him: for his glory has been upon me, wherefore I can judge between him and thee. Depart hence, Satan.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:19} % *MOSES-1-19 
		% ---
			\label{MOSES-1-19}

			And now, when Moses had said these words, Satan cried with a loud voice, and ranted upon the earth, and commanded, saying: I am the Only Begotten, worship me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:20} % *MOSES-1-20 
		% ---
			\label{MOSES-1-20}

			And it came to pass that Moses began to fear exceedingly; and as he began to fear, he saw the bitterness of hell. Nevertheless, calling upon God, he received strength, and he commanded, saying: Depart from me, Satan, for this one God only will I worship, which is the God of glory.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:21} % *MOSES-1-21 
		% ---
			\label{MOSES-1-21}

			And now Satan began to tremble, and the earth shook; and Moses received strength, and called upon God, saying: In the name of the Only Begotten, depart hence, Satan.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:22} % *MOSES-1-22 
		% ---
			\label{MOSES-1-22}

			And it came to pass that Satan cried with a loud voice, with weeping, and wailing, and gnashing of teeth; and he departed hence, even from the presence of Moses, that he beheld him not.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:23} % *MOSES-1-23 
		% ---
			\label{MOSES-1-23}

			And now of this thing Moses bore record; but because of wickedness it is not had among the children of men.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:24} % *MOSES-1-24 
		% ---
			\label{MOSES-1-24}

			And it came to pass that when Satan had departed from the presence of Moses, that Moses lifted up his eyes unto heaven, being filled with the Holy Ghost, which beareth record of the Father and the Son;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:25} % *MOSES-1-25 
		% ---
			\label{MOSES-1-25}

			And calling upon the name of God, he beheld his glory again, for it was upon him; and he heard a voice, saying: Blessed art thou, Moses, for I, the Almighty, have chosen thee, and thou shalt be made stronger than many waters; for they shall obey thy command as if thou wert God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:26} % *MOSES-1-26 
		% ---
			\label{MOSES-1-26}

			And lo, I am with thee, even unto the end of thy days; for thou shalt deliver my people from bondage, even Israel my chosen.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:27} % *MOSES-1-27 
		% ---
			\label{MOSES-1-27}

			And it came to pass, as the voice was still speaking, Moses cast his eyes and beheld the earth, yea, even all of it; and there was not a particle of it which he did not behold, discerning it by the spirit of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:28} % *MOSES-1-28 
		% ---
			\label{MOSES-1-28}

			And he beheld also the inhabitants thereof, and there was not a soul which he beheld not; and he discerned them by the Spirit of God; and their numbers were great, even numberless as the sand upon the sea shore.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:29} % *MOSES-1-29 
		% ---
			\label{MOSES-1-29}

			And he beheld many lands; and each land was called earth, and there were inhabitants on the face thereof.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:30} % *MOSES-1-30 
		% ---
			\label{MOSES-1-30}

			And it came to pass that Moses called upon God, saying: Tell me, I pray thee, why these things are so, and by what thou madest them?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:31} % *MOSES-1-31 
		% ---
			\label{MOSES-1-31}

			And behold, the glory of the Lord was upon Moses, so that Moses stood in the presence of God, and talked with him face to face. And the Lord God said unto Moses: For mine own purpose have I made these things. Here is wisdom and it remaineth in me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:32} % *MOSES-1-32 
		% ---
			\label{MOSES-1-32}

			And by the word of my power, have I created them, which is mine Only Begotten Son, who is full of grace and truth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:33} % *MOSES-1-33 
		% ---
			\label{MOSES-1-33}

			And worlds without number have I created; and I also created them for mine own purpose; and by the Son I created them, which is mine Only Begotten.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:34} % *MOSES-1-34 
		% ---
			\label{MOSES-1-34}

			And the first man of all men have I called Adam, which is many.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:35} % *MOSES-1-35 
		% ---
			\label{MOSES-1-35}

			But only an account of this earth, and the inhabitants thereof, give I unto you. For behold, there are many worlds that have passed away by the word of my power. And there are many that now stand, and innumerable are they unto man; but all things are numbered unto me, for they are mine and I know them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:36} % *MOSES-1-36 
		% ---
			\label{MOSES-1-36}

			And it came to pass that Moses spake unto the Lord, saying: Be merciful unto thy servant, O God, and tell me concerning this earth, and the inhabitants thereof, and also the heavens, and then thy servant will be content.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:37} % *MOSES-1-37 
		% ---
			\label{MOSES-1-37}

			And the Lord God spake unto Moses, saying: The heavens, they are many, and they cannot be numbered unto man; but they are numbered unto me, for they are mine.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:38} % *MOSES-1-38 
		% ---
			\label{MOSES-1-38}

			And as one earth shall pass away, and the heavens thereof even so shall another come, and there is no end to my works, neither to my words.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% --
		\paragraph{Moses 1:39} % *MOSES-1-39
		% ---
			\label{MOSES-1-39}
			
				\footnote{%
					% \label{}%
					Compare \hyperref[1NEPHI-14-7]{1 Nephi 14:7}, 
					}%
			For behold, this is my work and my glory---to bring to pass the immortality and eternal life of man.
			
			\begin{framed}
			\scriptsize
				\begin{compactdesc}
					\item[OTR1] for behold this is my work to my glory to the immortality \& the eternal life of man
					\item[OTRJW] for behold this is my work to my glory to the immortality \& the Eternal life of man
					\item[OTR2] for Behold this is my work \sout{to} <and> my glory to \sout{the} <bring to pass the> immortality \& eternal life of man.
				\end{compactdesc}
			\end{framed}

		% ---
		\paragraph{Moses 1:40} % *MOSES-1-40 
		% ---
			\label{MOSES-1-40}

			And now, Moses, my son, I will speak unto thee concerning this earth upon which thou standest; and thou shalt write the things which I shall speak.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:41} % *MOSES-1-41 
		% ---
			\label{MOSES-1-41}

			And in a day when the children of men shall esteem my words as naught and take many of them from the book which thou shalt write, behold, I will raise up another like unto thee; and they shall be had again among the children of men---among as many as shall believe.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 1:42} % *MOSES-1-42
		% ---
			\label{MOSES-1-42}

			(These words were spoken unto Moses in the mount, the name of which shall not be known among the children of men. And now they are spoken unto you. Show them not unto any except them that believe. Even so. Amen.)

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
			
	% -----
	\subsubsection{Moses 2} % *MOSES-2
	% -----
		\label{MOSES-2}
		
		% ---
		\paragraph{Moses 2:1} % *MOSES-2-1 
		% ---
			\label{MOSES-2-1}

			AND it came to pass that the Lord spake unto Moses, saying: Behold, I reveal unto you concerning this heaven, and this earth; write the words which I speak. I am the Beginning and the End, the Almighty God; by mine Only Begotten I created these things; yea, in the beginning I created the heaven, and the earth upon which thou standest.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:2} % *MOSES-2-2 
		% ---
			\label{MOSES-2-2}

			And the earth was without form, and void; and I caused darkness to come up upon the face of the deep; and my Spirit moved upon the face of the water; for I am God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:3} % *MOSES-2-3 
		% ---
			\label{MOSES-2-3}

			And I, God, said: Let there be light; and there was light.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:4} % *MOSES-2-4 
		% ---
			\label{MOSES-2-4}

			And I, God, saw the light; and that light was good. And I, God, divided the light from the darkness.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:5} % *MOSES-2-5 
		% ---
			\label{MOSES-2-5}

			And I, God, called the light Day; and the darkness, I called Night; and this I did by the word of my power, and it was done as I spake; and the evening and the morning were the first day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:6} % *MOSES-2-6 
		% ---
			\label{MOSES-2-6}

			And again, I, God, said: Let there be a firmament in the midst of the water, and it was so, even as I spake; and I said: Let it divide the waters from the waters; and it was done;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:7} % *MOSES-2-7 
		% ---
			\label{MOSES-2-7}

			And I, God, made the firmament and divided the waters, yea, the great waters under the firmament from the waters which were above the firmament, and it was so even as I spake.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:8} % *MOSES-2-8 
		% ---
			\label{MOSES-2-8}

			And I, God, called the firmament Heaven; and the evening and the morning were the second day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:9} % *MOSES-2-9 
		% ---
			\label{MOSES-2-9}

			And I, God, said: Let the waters under the heaven be gathered together unto one place, and it was so; and I, God, said: Let there be dry land; and it was so.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:10} % *MOSES-2-10 
		% ---
			\label{MOSES-2-10}

			And I, God, called the dry land Earth; and the gathering together of the waters, called I the Sea; and I, God, saw that all things which I had made were good.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:11} % *MOSES-2-11 
		% ---
			\label{MOSES-2-11}

			And I, God, said: Let the earth bring forth grass, the herb yielding seed, the fruit tree yielding fruit, after his kind, and the tree yielding fruit, whose seed should be in itself upon the earth, and it was so even as I spake.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:12} % *MOSES-2-12 
		% ---
			\label{MOSES-2-12}

			And the earth brought forth grass, every herb yielding seed after his kind, and the tree yielding fruit, whose seed should be in itself, after his kind; and I, God, saw that all things which I had made were good;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:13} % *MOSES-2-13 
		% ---
			\label{MOSES-2-13}

			And the evening and the morning were the third day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:14} % *MOSES-2-14 
		% ---
			\label{MOSES-2-14}

			And I, God, said: Let there be lights in the firmament of the heaven, to divide the day from the night, and let them be for signs, and for seasons, and for days, and for years;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:15} % *MOSES-2-15 
		% ---
			\label{MOSES-2-15}

			And let them be for lights in the firmament of the heaven to give light upon the earth; and it was so.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:16} % *MOSES-2-16 
		% ---
			\label{MOSES-2-16}

			And I, God, made two great lights; the greater light to rule the day, and the lesser light to rule the night, and the greater light was the sun, and the lesser light was the moon; and the stars also were made even according to my word.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:17} % *MOSES-2-17 
		% ---
			\label{MOSES-2-17}

			And I, God, set them in the firmament of the heaven to give light upon the earth,

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:18} % *MOSES-2-18 
		% ---
			\label{MOSES-2-18}

			And the sun to rule over the day, and the moon to rule over the night, and to divide the light from the darkness; and I, God, saw that all things which I had made were good;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:19} % *MOSES-2-19 
		% ---
			\label{MOSES-2-19}

			And the evening and the morning were the fourth day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:20} % *MOSES-2-20 
		% ---
			\label{MOSES-2-20}

			And I, God, said: Let the waters bring forth abundantly the moving creature that hath life, and fowl which may fly above the earth in the open firmament of heaven.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:21} % *MOSES-2-21 
		% ---
			\label{MOSES-2-21}

			And I, God, created great whales, and every living creature that moveth, which the waters brought forth abundantly, after their kind, and every winged fowl after his kind; and I, God, saw that all things which I had created were good.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:22} % *MOSES-2-22 
		% ---
			\label{MOSES-2-22}

			And I, God, blessed them, saying: Be fruitful, and multiply, and fill the waters in the sea; and let fowl multiply in the earth;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:23} % *MOSES-2-23 
		% ---
			\label{MOSES-2-23}

			And the evening and the morning were the fifth day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:24} % *MOSES-2-24 
		% ---
			\label{MOSES-2-24}

			And I, God, said: Let the earth bring forth the living creature after his kind, cattle, and creeping things, and beasts of the earth after their kind, and it was so;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:25} % *MOSES-2-25 
		% ---
			\label{MOSES-2-25}

			And I, God, made the beasts of the earth after their kind, and cattle after their kind, and everything which creepeth upon the earth after his kind; and I, God, saw that all these things were good.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:26} % *MOSES-2-26 
		% ---
			\label{MOSES-2-26}

			And I, God, said unto mine Only Begotten, which was with me from the beginning: Let us make man in our image, after our likeness; and it was so. And I, God, said: Let them have dominion over the fishes of the sea, and over the fowl of the air, and over the cattle, and over all the earth, and over every creeping thing that creepeth upon the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:27} % *MOSES-2-27 
		% ---
			\label{MOSES-2-27}

			And I, God, created man in mine own image, in the image of mine Only Begotten created I him; male and female created I them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:28} % *MOSES-2-28 
		% ---
			\label{MOSES-2-28}

			And I, God, blessed them, and said unto them: Be fruitful, and multiply, and replenish the earth, and subdue it, and have dominion over the fish of the sea, and over the fowl of the air, and over every living thing that moveth upon the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:29} % *MOSES-2-29 
		% ---
			\label{MOSES-2-29}

			And I, God, said unto man: Behold, I have given you every herb bearing seed, which is upon the face of all the earth, and every tree in the which shall be the fruit of a tree yielding seed; to you it shall be for meat.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:30} % *MOSES-2-30 
		% ---
			\label{MOSES-2-30}

			And to every beast of the earth, and to every fowl of the air, and to everything that creepeth upon the earth, wherein I grant life, there shall be given every clean herb for meat; and it was so, even as I spake.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 2:31} % *MOSES-2-31 
		% ---
			\label{MOSES-2-31}

			And I, God, saw everything that I had made, and, behold, all things which I had made were very good; and the evening and the morning were the sixth day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 3} % *MOSES-3
	% -----
		\label{MOSES-3}
		
		% ---
		\paragraph{Moses 3:1} % *MOSES-3-1 
		% ---
			\label{MOSES-3-1}

			THUS the heaven and the earth were finished, and all the host of them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:2} % *MOSES-3-2 
		% ---
			\label{MOSES-3-2}

			And on the seventh day I, God, ended my work, and all things which I had made; and I rested on the seventh day from all my work, and all things which I had made were finished, and I, God, saw that they were good;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]	
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:3} % *MOSES-3-3 
		% ---
			\label{MOSES-3-3}

			And I, God, blessed the seventh day, and sanctified it; because that in it I had rested from all my work which I, God, had created and made.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:4} % *MOSES-3-4 
		% ---
			\label{MOSES-3-4}

			And now, behold, I say unto you, that these are the generations of the heaven and of the earth, when they were created, in the day that I, the Lord God, made the heaven and the earth,

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:5} % *MOSES-3-5 
		% ---
			\label{MOSES-3-5}

			And every plant of the field before it was in the earth, and every herb of the field before it grew. For I, the Lord God, created all things, of which I have spoken, spiritually,%
				\footnote{%
					% \label{}%
					For this ``spiritual creation,'' see \hyperref[DC29-31]{DC 29:31--44}. Note especially verse 34, where the Lord seems to suggest that the distinction between ``spiritual'' and ``temporal'' doesn't really mean anything to him, and that this is a way of talking that is supposed to help us ``naturally understand'' (29:33).
					}
			before they were naturally upon the face of the earth. For I, the Lord God, had not caused it to rain upon the face of the earth. And I, the Lord God, had created all the children of men; and not yet a man to till the ground; for in heaven created I them; and there was not yet flesh upon the earth, neither in the water, neither in the air;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:6} % *MOSES-3-6 
		% ---
			\label{MOSES-3-6}

			But I, the Lord God, spake, and there went up a mist from the earth, and watered the whole face of the ground.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:7} % *MOSES-3-7 
		% ---
			\label{MOSES-3-7}

			And I, the Lord God, formed man from the dust of the ground, and breathed into his nostrils the breath of life; and man became a living soul, the first flesh upon the earth, the first man also; nevertheless, all things were before created; but spiritually were they created and made according to my word.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:8} % *MOSES-3-8 
		% ---
			\label{MOSES-3-8}

			And I, the Lord God, planted a garden eastward in Eden, and there I put the man whom I had formed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:9} % *MOSES-3-9 
		% ---
			\label{MOSES-3-9}

			And out of the ground made I, the Lord God, to grow every tree, naturally, that is pleasant to the sight of man; and man could behold it. And it became also a living soul. For it was spiritual in the day that I created it; for it remaineth in the sphere in which I, God, created it, yea, even all things which I prepared for the use of man; and man saw that it was good for food. And I, the Lord God, planted the tree of life also in the midst of the garden, and also the tree of knowledge of good and evil.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:10} % *MOSES-3-10 
		% ---
			\label{MOSES-3-10}

			And I, the Lord God, caused a river to go out of Eden to water the garden; and from thence it was parted, and became into four heads.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:11} % *MOSES-3-11 
		% ---
			\label{MOSES-3-11}

			And I, the Lord God, called the name of the first Pison, and it compasseth the whole land of Havilah, where I, the Lord God, created much gold;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:12} % *MOSES-3-12 
		% ---
			\label{MOSES-3-12}

			And the gold of that land was good, and there was bdellium and the onyx stone.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:13} % *MOSES-3-13 
		% ---
			\label{MOSES-3-13}

			And the name of the second river was called Gihon; the same that compasseth the whole land of Ethiopia.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:14} % *MOSES-3-14 
		% ---
			\label{MOSES-3-14}

			And the name of the third river was Hiddekel; that which goeth toward the east of Assyria. And the fourth river was the Euphrates.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:15} % *MOSES-3-15 
		% ---
			\label{MOSES-3-15}

			And I, the Lord God, took the man, and put him into the Garden of Eden, to dress it, and to keep it.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:16} % *MOSES-3-16 
		% ---
			\label{MOSES-3-16}

			And I, the Lord God, commanded the man, saying: Of every tree of the garden thou mayest freely eat,

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:17} % *MOSES-3-17 
		% ---
			\label{MOSES-3-17}

			But of the tree of the knowledge of good and evil, thou shalt not eat of it, nevertheless, thou mayest choose for thyself, for it is given unto thee; but, remember that I forbid it, for in the day thou eatest thereof thou shalt surely die.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:18} % *MOSES-3-18 
		% ---
			\label{MOSES-3-18}

			And I, the Lord God, said unto mine Only Begotten, that it was not good that the man should be alone; wherefore, I will make an help meet for him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:19} % *MOSES-3-19 
		% ---
			\label{MOSES-3-19}

			And out of the ground I, the Lord God, formed every beast of the field, and every fowl of the air; and commanded that they should come unto Adam, to see what he would call them; and they were also living souls; for I, God, breathed into them the breath of life, and commanded that whatsoever Adam called every living creature, that should be the name thereof.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:20} % *MOSES-3-20 
		% ---
			\label{MOSES-3-20}

			And Adam gave names to all cattle, and to the fowl of the air, and to every beast of the field; but as for Adam, there was not found an help meet for him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:21} % *MOSES-3-21 
		% ---
			\label{MOSES-3-21}

			And I, the Lord God, caused a deep sleep to fall upon Adam; and he slept, and I took one of his ribs and closed up the flesh in the stead thereof;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:22} % *MOSES-3-22 
		% ---
			\label{MOSES-3-22}

			And the rib which I, the Lord God, had taken from man, made I a woman, and brought her unto the man.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:23} % *MOSES-3-23 
		% ---
			\label{MOSES-3-23}

			And Adam said: This I know now is bone of my bones, and flesh of my flesh; she shall be called Woman, because she was taken out of man.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:24} % *MOSES-3-24 
		% ---
			\label{MOSES-3-24}

			Therefore shall a man leave his father and his mother, and shall cleave unto his wife; and they shall be one flesh.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 3:25} % *MOSES-3-25 
		% ---
			\label{MOSES-3-25}

			And they were both naked, the man and his wife, and were not ashamed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 4} % *MOSES-4
	% -----
		\label{MOSES-4}
		
		% ---
		\paragraph{Moses 4:1} % *MOSES-4-1 
		% ---
			\label{MOSES-4-1}

				\footnote{%
					% \label{}%
					We have to read this chapter in conjunction with \hyperref[DC29]{DC 29}.
					}%
			AND I, the Lord God, spake unto Moses, saying: That Satan, whom thou hast commanded in the name of mine Only Begotten, is the same which was from the beginning, and he came before me, saying---Behold, here am I, send me, I will be thy son,%
				\footnote{%
					% \label{}%
					Christ was the ``only begotten,'' or \gr{μονογενής}, of the Father; this means he had a special relationship with the Father, such that Christ received his power directly from the Father---see \hyperref[JOHN-5-26]{John 5:26}, \hyperref[JOHN-14-10]{John 14:10}, \hyperref[3NEPHI-9-15]{3 Nephi 9:15}, \hyperref[3NEPHI-11-27]{3 Nephi 11:27}, \hyperref[DC35-2]{DC 35:2}, \hyperref[DC50-43]{DC 50:43}, and \hyperref[DC93-3]{DC 93:3}. Satan wanted that special relationship with the Father. (We do not have this relationship with the Father---we have it with Christ. This is why we become the ``\hyperref[NEWBIRTH-children-christ]{children of Christ},'' and why the true name of the priesthood on this planet in this church is ``the Holy Priesthood, after the Order of the Son of God''; see \hyperref[DC107-3]{DC 107:3}.)
					}
			and I will redeem all mankind, that one soul shall not be lost, and surely I will do it; wherefore give me thine
				\footnote{%
					% \label{}%
					See \hyperref[DC29-36]{DC 29:36} and note especially the connection to \hyperref[HEBREWS-5-4]{Hebrews 5:4}; DC 29 is September 1830, and Moses 4 is \hyperref[JST-TIMELINE]{around the same time}. I take ``honor'' here to refer to priesthood power and the office associated with having certain power in the priesthood. Notice that Satan wants to do it all himself and take all the credit: ``Here \textit{I} am, send \textit{me}, \textit{I} will be thy son, and \textit{I} will redeem all mankind,'' etc. He definitely wanted the ``honor'' part of the priesthood. See \hyperref[MARK-12-7]{Mark 12:7}, \hyperref[JOHN-5-23]{John 5:23}, 
					}%
			honor.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:2} % *MOSES-4-2 
		% ---
			\label{MOSES-4-2}

			But, behold, my Beloved Son, which was my Beloved and Chosen from the beginning, said unto me---Father, thy will be done, and the glory be thine forever.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:3} % *MOSES-4-3 
		% ---
			\label{MOSES-4-3}

			Wherefore, because that Satan rebelled against me, and sought to destroy the agency of man, which I, the Lord God, had given him, and also, that I should give unto him mine own power; by the power of mine Only Begotten, I caused that he should be cast down;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:4} % *MOSES-4-4 
		% ---
			\label{MOSES-4-4}

			And he became Satan, yea, even the devil, the father of all lies, to deceive and to blind men, and to lead them captive at his will, even as many as would not hearken unto my voice.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:5} % *MOSES-4-5 
		% ---
			\label{MOSES-4-5}

			And now the serpent was more subtle than any beast of the field which I, the Lord God, had made.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:6} % *MOSES-4-6 
		% ---
			\label{MOSES-4-6}

			And Satan put it into the heart of the serpent, (for he had drawn away many after him,) and he sought also to beguile Eve, for he knew not the mind of God, wherefore he sought to destroy the world.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:7} % *MOSES-4-7 
		% ---
			\label{MOSES-4-7}

			And he said unto the woman: Yea, hath God said---Ye shall not eat of every tree of the garden? (And he spake by the mouth of the serpent.)

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:8} % *MOSES-4-8 
		% ---
			\label{MOSES-4-8}

			And the woman said unto the serpent: We may eat of the fruit of the trees of the garden;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:9} % *MOSES-4-9 
		% ---
			\label{MOSES-4-9}

			But of the fruit of the tree which thou beholdest in the midst of the garden, God hath said---Ye shall not eat of it, neither shall ye touch it, lest ye die.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:10} % *MOSES-4-10 
		% ---
			\label{MOSES-4-10}

			And the serpent said unto the woman: Ye shall not surely die;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:11} % *MOSES-4-11 
		% ---
			\label{MOSES-4-11}

			For God doth know that in the day ye eat thereof, then your eyes shall be opened, and ye shall be as gods, knowing good and evil.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:12} % *MOSES-4-12 
		% ---
			\label{MOSES-4-12}

			And when the woman saw that the tree was good for food, and that it became pleasant to the eyes, and a tree to be desired to make her wise, she took of the fruit thereof, and did eat, and also gave unto her husband with her, and he did eat.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:13} % *MOSES-4-13 
		% ---
			\label{MOSES-4-13}

			And the eyes of them both were opened, and they knew that they had been naked. And they sewed fig-leaves together and made themselves aprons.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:14} % *MOSES-4-14 
		% ---
			\label{MOSES-4-14}

			And they heard the voice of the Lord God, as they were walking in the garden, in the cool of the day; and Adam and his wife went to hide themselves from the presence of the Lord God amongst the trees of the garden.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:15} % *MOSES-4-15 
		% ---
			\label{MOSES-4-15}

			And I, the Lord God, called unto Adam, and said unto him: Where goest thou?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:16} % *MOSES-4-16 
		% ---
			\label{MOSES-4-16}

			And he said: I heard thy voice in the garden, and I was afraid, because I beheld that I was naked, and I hid myself.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:17} % *MOSES-4-17 
		% ---
			\label{MOSES-4-17}

			And I, the Lord God, said unto Adam: Who told thee thou wast naked? Hast thou eaten of the tree whereof I commanded thee that thou shouldst not eat, if so thou shouldst surely die?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:18} % *MOSES-4-18 
		% ---
			\label{MOSES-4-18}

			And the man said: The woman thou gavest me, and commandest that she should remain with me, she gave me of the fruit of the tree and I did eat.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:19} % *MOSES-4-19 
		% ---
			\label{MOSES-4-19}

			And I, the Lord God, said unto the woman: What is this thing which thou hast done? And the woman said: The serpent beguiled me, and I did eat.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:20} % *MOSES-4-20 
		% ---
			\label{MOSES-4-20}

			And I, the Lord God, said unto the serpent: Because thou hast done this thou shalt be cursed above all cattle, and above every beast of the field; upon thy belly shalt thou go, and dust shalt thou eat all the days of thy life;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:21} % *MOSES-4-21 
		% ---
			\label{MOSES-4-21}

			And I will put enmity between thee and the woman, between thy seed and her seed; and he shall bruise thy head, and thou shalt bruise his heel.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:22} % *MOSES-4-22 
		% ---
			\label{MOSES-4-22}

			Unto the woman, I, the Lord God, said: I will greatly multiply thy sorrow and thy conception. In sorrow thou shalt bring forth children, and thy desire shall be to thy husband, and he shall rule over thee.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:23} % *MOSES-4-23 
		% ---
			\label{MOSES-4-23}

			And unto Adam, I, the Lord God, said: Because thou hast hearkened unto the voice of thy wife, and hast eaten of the fruit of the tree of which I commanded thee, saying---Thou shalt not eat of it, cursed shall be the ground for thy sake; in sorrow shalt thou eat of it all the days of thy life.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:24} % *MOSES-4-24 
		% ---
			\label{MOSES-4-24}

			Thorns also, and thistles shall it bring forth to thee, and thou shalt eat the herb of the field.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:25} % *MOSES-4-25 
		% ---
			\label{MOSES-4-25}

			By the sweat of thy face shalt thou eat bread, until thou shalt return unto the ground---for thou shalt surely die---for out of it wast thou taken: for dust thou wast, and unto dust shalt thou return.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:26} % *MOSES-4-26 
		% ---
			\label{MOSES-4-26}

			And Adam called his wife's name Eve, because she was the mother of all living; for thus have I, the Lord God, called the first of all women, which are many.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:27} % *MOSES-4-27 
		% ---
			\label{MOSES-4-27}

			Unto Adam, and also unto his wife, did I, the Lord God, make coats of skins, and clothed them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:28} % *MOSES-4-28 
		% ---
			\label{MOSES-4-28}

			And I, the Lord God, said unto mine Only Begotten: Behold, the man is become as one of us to know good and evil; and now lest he put forth his hand and partake also of the tree of life, and eat and live forever,

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:29} % *MOSES-4-29 
		% ---
			\label{MOSES-4-29}

			Therefore I, the Lord God, will send him forth from the Garden of Eden, to till the ground from whence he was taken;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:30} % *MOSES-4-30 
		% ---
			\label{MOSES-4-30}

			For as I, the Lord God, liveth, even so my words cannot return void, for as they go forth out of my mouth they must be fulfilled.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:31} % *MOSES-4-31 
		% ---
			\label{MOSES-4-31}

			So I drove out the man, and I placed at the east of the Garden of Eden, cherubim and a flaming sword, which turned every way to keep the way of the tree of life.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 4:32} % *MOSES-4-32 
		% ---
			\label{MOSES-4-32}

			(And these are the words which I spake unto my servant Moses, and they are true even as I will; and I have spoken them unto you. See thou show them unto no man, until I command you, except to them that believe. Amen.)

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 5} % *MOSES-5
	% -----
		\label{MOSES-5}
		
		% ---
		\paragraph{Moses 5:1} % *MOSES-5-1 
		% ---
			\label{MOSES-5-1}

			AND it came to pass that after I, the Lord God, had driven them out, that Adam began to till the earth, and to have dominion over all the beasts of the field, and to eat his bread by the sweat of his brow, as I the Lord had commanded him. And Eve, also, his wife, did labor with him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:2} % *MOSES-5-2 
		% ---
			\label{MOSES-5-2}

			And Adam knew his wife, and she bare unto him sons and daughters, and they began to multiply and to replenish the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:3} % *MOSES-5-3 
		% ---
			\label{MOSES-5-3}

			And from that time forth, the sons and daughters of Adam began to divide two and two in the land, and to till the land, and to tend flocks, and they also begat sons and daughters.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:4} % *MOSES-5-4 
		% ---
			\label{MOSES-5-4}

			And Adam and Eve, his wife, called upon the name of the Lord, and they heard the voice of the Lord from the way toward the Garden of Eden, speaking unto them, and they saw him not; for they were shut out from his presence.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:5} % *MOSES-5-5 
		% ---
			\label{MOSES-5-5}

			And he gave unto them commandments, that they should worship the Lord their God, and should offer the firstlings of their flocks, for an offering unto the Lord. And Adam was obedient unto the commandments of the Lord.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:6} % *MOSES-5-6 
		% ---
			\label{MOSES-5-6}

			And after many days an angel of the Lord appeared unto Adam, saying: Why dost thou offer sacrifices unto the Lord? And Adam said unto him: I know not, save the Lord commanded me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:7} % *MOSES-5-7 
		% ---
			\label{MOSES-5-7}

			And then the angel spake, saying: This thing is a similitude of the sacrifice of the Only Begotten of the Father, which is full of grace and truth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:8} % *MOSES-5-8 
		% ---
			\label{MOSES-5-8}

			Wherefore, thou shalt do all that thou doest in the name of the Son, and thou shalt repent and call upon God in the name of the Son forevermore.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:9} % *MOSES-5-9 
		% ---
			\label{MOSES-5-9}

			And in that day the Holy Ghost fell upon Adam, which beareth record of the Father and the Son, saying: I am the Only Begotten of the Father from the beginning, henceforth and forever, that as thou hast fallen thou mayest be redeemed, and all mankind, even as many as will.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:10} % *MOSES-5-10 
		% ---
			\label{MOSES-5-10}

			And in that day Adam blessed God and was filled, and began to prophesy concerning all the families of the earth, saying: Blessed be the name of God, for because of my transgression my eyes are opened, and in this life I shall have joy, and again in the flesh I shall see God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:11} % *MOSES-5-11 
		% ---
			\label{MOSES-5-11}

			And Eve, his wife, heard all these things and was glad, saying: Were it not for our transgression we never should have had seed, and never should have known good and evil, and the joy of our redemption, and the eternal life which God giveth unto all the obedient.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:12} % *MOSES-5-12 
		% ---
			\label{MOSES-5-12}

			And Adam and Eve blessed the name of God, and they made all things known unto their sons and their daughters.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:13} % *MOSES-5-13 
		% ---
			\label{MOSES-5-13}

			And Satan came among them, saying: I am also a son of God; and he commanded them, saying: Believe it not; and they believed it not, and they loved Satan more than God. And men began from that time forth to be carnal, sensual, and devilish.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:14} % *MOSES-5-14 
		% ---
			\label{MOSES-5-14}

			And the Lord God called upon men by the Holy Ghost everywhere and commanded them that they should repent;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:15} % *MOSES-5-15 
		% ---
			\label{MOSES-5-15}

			And as many as believed in the Son, and repented of their sins, should be saved; and as many as believed not and repented not, should be damned; and the words went forth out of the mouth of God in a firm decree; wherefore they must be fulfilled.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:16} % *MOSES-5-16 
		% ---
			\label{MOSES-5-16}

			And Adam and Eve, his wife, ceased not to call upon God. And Adam knew Eve his wife, and she conceived and bare Cain, and said: I have gotten a man from the Lord; wherefore he may not reject his words. But behold, Cain hearkened not, saying: Who is the Lord that I should know him?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:17} % *MOSES-5-17 
		% ---
			\label{MOSES-5-17}

			And she again conceived and bare his brother Abel. And Abel hearkened unto the voice of the Lord. And Abel was a keeper of sheep, but Cain was a tiller of the ground.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:18} % *MOSES-5-18 
		% ---
			\label{MOSES-5-18}

			And Cain loved Satan more than God. And Satan commanded him, saying: Make an offering unto the Lord.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:19} % *MOSES-5-19 
		% ---
			\label{MOSES-5-19}

			And in process of time it came to pass that Cain brought of the fruit of the ground an offering unto the Lord.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:20} % *MOSES-5-20 
		% ---
			\label{MOSES-5-20}

			And Abel he also brought of the firstlings of his flock, and of the fat thereof. And the Lord had respect unto Abel, and to his offering;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:21} % *MOSES-5-21 
		% ---
			\label{MOSES-5-21}

			But unto Cain, and to his offering, he had not respect. Now Satan knew this, and it pleased him. And Cain was very wroth, and his countenance fell.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:22} % *MOSES-5-22 
		% ---
			\label{MOSES-5-22}

			And the Lord said unto Cain: Why art thou wroth? Why is thy countenance fallen?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:23} % *MOSES-5-23 
		% ---
			\label{MOSES-5-23}

			If thou doest well, thou shalt be accepted. And if thou doest not well, sin lieth at the door, and Satan desireth to have thee; and except thou shalt hearken unto my commandments, I will deliver thee up, and it shall be unto thee according to his desire. And thou shalt rule over him;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:24} % *MOSES-5-24 
		% ---
			\label{MOSES-5-24}

			For from this time forth thou shalt be the father of his lies; thou shalt be called Perdition; for thou wast also before the world.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:25} % *MOSES-5-25 
		% ---
			\label{MOSES-5-25}

			And it shall be said in time to come---That these abominations were had from Cain; for he rejected the greater counsel which was had from God; and this is a cursing which I will put upon thee, except thou repent.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:26} % *MOSES-5-26 
		% ---
			\label{MOSES-5-26}

			And Cain was wroth, and listened not any more to the voice of the Lord, neither to Abel, his brother, who walked in holiness before the Lord.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:27} % *MOSES-5-27 
		% ---
			\label{MOSES-5-27}

			And Adam and his wife mourned before the Lord, because of Cain and his brethren.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:28} % *MOSES-5-28 
		% ---
			\label{MOSES-5-28}

			And it came to pass that Cain took one of his brothers' daughters to wife, and they loved Satan more than God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:29} % *MOSES-5-29 
		% ---
			\label{MOSES-5-29}

			And Satan said unto Cain: Swear unto me by thy throat, and if thou tell it thou shalt die; and swear thy brethren by their heads, and by the living God, that they tell it not; for if they tell it, they shall surely die; and this that thy father may not know it; and this day I will deliver thy brother Abel into thine hands.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:30} % *MOSES-5-30 
		% ---
			\label{MOSES-5-30}

			And Satan sware unto Cain that he would do according to his commands. And all these things were done in secret.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:31} % *MOSES-5-31 
		% ---
			\label{MOSES-5-31}

			And Cain said: Truly I am Mahan, the master of this great secret, that I may murder and get gain. Wherefore Cain was called Master Mahan, and he gloried in his wickedness.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:32} % *MOSES-5-32 
		% ---
			\label{MOSES-5-32}

			And Cain went into the field, and Cain talked with Abel, his brother. And it came to pass that while they were in the field, Cain rose up against Abel, his brother, and slew him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:33} % *MOSES-5-33 
		% ---
			\label{MOSES-5-33}

			And Cain gloried in that which he had done, saying: I am free; surely the flocks of my brother falleth into my hands.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:34} % *MOSES-5-34 
		% ---
			\label{MOSES-5-34}

			And the Lord said unto Cain: Where is Abel, thy brother? And he said: I know not. Am I my brother's keeper?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:35} % *MOSES-5-35 
		% ---
			\label{MOSES-5-35}

			And the Lord said: What hast thou done? The voice of thy brother's blood cries unto me from the ground.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:36} % *MOSES-5-36 
		% ---
			\label{MOSES-5-36}

			And now thou shalt be cursed from the earth which hath opened her mouth to receive thy brother's blood from thy hand.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:37} % *MOSES-5-37 
		% ---
			\label{MOSES-5-37}

			When thou tillest the ground it shall not henceforth yield unto thee her strength. A fugitive and a vagabond shalt thou be in the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:38} % *MOSES-5-38 
		% ---
			\label{MOSES-5-38}

			And Cain said unto the Lord: Satan tempted me because of my brother's flocks. And I was wroth also; for his offering thou didst accept and not mine; my punishment is greater than I can bear.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:39} % *MOSES-5-39 
		% ---
			\label{MOSES-5-39}

			Behold thou hast driven me out this day from the face of the Lord, and from thy face shall I be hid; and I shall be a fugitive and a vagabond in the earth; and it shall come to pass, that he that findeth me will slay me, because of mine iniquities, for these things are not hid from the Lord.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:40} % *MOSES-5-40 
		% ---
			\label{MOSES-5-40}

			And I the Lord said unto him: Whosoever slayeth thee, vengeance shall be taken on him sevenfold. And I the Lord set a mark upon Cain, lest any finding him should kill him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:41} % *MOSES-5-41 
		% ---
			\label{MOSES-5-41}

			And Cain was shut out from the presence of the Lord, and with his wife and many of his brethren dwelt in the land of Nod, on the east of Eden.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:42} % *MOSES-5-42 
		% ---
			\label{MOSES-5-42}

			And Cain knew his wife, and she conceived and bare Enoch, and he also begat many sons and daughters. And he builded a city, and he called the name of the city after the name of his son, Enoch.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:43} % *MOSES-5-43 
		% ---
			\label{MOSES-5-43}

			And unto Enoch was born Irad, and other sons and daughters. And Irad begat Mahujael, and other sons and daughters. And Mahujael begat Methusael, and other sons and daughters. And Methusael begat Lamech.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:44} % *MOSES-5-44 
		% ---
			\label{MOSES-5-44}

			And Lamech took unto himself two wives; the name of one being Adah, and the name of the other, Zillah.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:45} % *MOSES-5-45 
		% ---
			\label{MOSES-5-45}

			And Adah bare Jabal; he was the father of such as dwell in tents, and they were keepers of cattle; and his brother's name was Jubal, who was the father of all such as handle the harp and organ.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:46} % *MOSES-5-46 
		% ---
			\label{MOSES-5-46}

			And Zillah, she also bare Tubal Cain, an instructor of every artificer in brass and iron. And the sister of Tubal Cain was called Naamah.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:47} % *MOSES-5-47 
		% ---
			\label{MOSES-5-47}

			And Lamech said unto his wives, Adah and Zillah: Hear my voice, ye wives of Lamech, hearken unto my speech; for I have slain a man to my wounding, and a young man to my hurt.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:48} % *MOSES-5-48 
		% ---
			\label{MOSES-5-48}

			If Cain shall be avenged sevenfold, truly Lamech shall be seventy and seven fold;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:49} % *MOSES-5-49 
		% ---
			\label{MOSES-5-49}

			For Lamech having entered into a covenant with Satan, after the manner of Cain, wherein he became Master Mahan, master of that great secret which was administered unto Cain by Satan; and Irad, the son of Enoch, having known their secret, began to reveal it unto the sons of Adam;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:50} % *MOSES-5-50 
		% ---
			\label{MOSES-5-50}

			Wherefore Lamech, being angry, slew him, not like unto Cain, his brother Abel, for the sake of getting gain, but he slew him for the
				\footnote{%
					% \label{}%
					Compare \hyperref[MARK-6-26]{Mark 6:26}.
					}%
			oath's sake.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:51} % *MOSES-5-51 
		% ---
			\label{MOSES-5-51}

			For, from the days of Cain, there was a secret combination, and their works were in the dark, and they knew every man his brother.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:52} % *MOSES-5-52 
		% ---
			\label{MOSES-5-52}

			Wherefore the Lord cursed Lamech, and his house, and all them that had covenanted with Satan; for they kept not the commandments of God, and it displeased God, and he ministered not unto them, and their works were abominations, and began to spread among all the sons of men. And it was among the sons of men.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:53} % *MOSES-5-53 
		% ---
			\label{MOSES-5-53}

			And among the daughters of men these things were not spoken, because that Lamech had spoken the secret unto his wives, and they rebelled against him, and declared these things abroad, and had not compassion;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:54} % *MOSES-5-54 
		% ---
			\label{MOSES-5-54}

			Wherefore Lamech was despised, and cast out, and came not among the sons of men, lest he should die.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:55} % *MOSES-5-55 
		% ---
			\label{MOSES-5-55}

			And thus the works of darkness began to prevail among all the sons of men.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:56} % *MOSES-5-56 
		% ---
			\label{MOSES-5-56}

			And God cursed the earth with a sore curse, and was angry with the wicked, with all the sons of men whom he had made;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:57} % *MOSES-5-57 
		% ---
			\label{MOSES-5-57}

			For they would not hearken unto his voice, nor believe on his Only Begotten Son, even him whom he declared should come in the meridian of time, who was prepared from before the foundation of the world.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:58} % *MOSES-5-58 
		% ---
			\label{MOSES-5-58}

			And thus the Gospel began to be preached, from the beginning, being declared by holy angels sent forth from the presence of God, and by his own voice, and by the gift of the Holy Ghost.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 5:59} % *MOSES-5-59 
		% ---
			\label{MOSES-5-59}

			And thus all things were confirmed unto Adam, by an holy ordinance, and the Gospel preached, and a decree sent forth, that it should be in the world, until the end thereof; and thus it was. Amen.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 6} % *MOSES-6
	% -----
		\label{MOSES-6}
		
		% ---
		\paragraph{Moses 6:1} % *MOSES-6-1 
		% ---
			\label{MOSES-6-1}

			AND Adam hearkened unto the voice of God, and called upon his sons to repent.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:2} % *MOSES-6-2 
		% ---
			\label{MOSES-6-2}

			And Adam knew his wife again, and she bare a son, and he called his name Seth. And Adam glorified the name of God; for he said: God hath appointed me another seed, instead of Abel, whom Cain slew.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:3} % *MOSES-6-3 
		% ---
			\label{MOSES-6-3}

			And God revealed himself unto Seth, and he rebelled not, but offered an acceptable sacrifice, like unto his brother Abel. And to him also was born a son, and he called his name Enos.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:4} % *MOSES-6-4 
		% ---
			\label{MOSES-6-4}

			And then began these men to call upon the name of the Lord, and the Lord blessed them;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:5} % *MOSES-6-5 
		% ---
			\label{MOSES-6-5}

			And a book of remembrance was kept, in the which was recorded, in the language of Adam, for it was given unto as many as called upon God to write by the spirit of inspiration;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:6} % *MOSES-6-6 
		% ---
			\label{MOSES-6-6}

			And by them their children were taught to read and write, having a language which was pure and undefiled.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:7} % *MOSES-6-7 
		% ---
			\label{MOSES-6-7}

			Now this same Priesthood, which was in the beginning, shall be in the end of the world also.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:8} % *MOSES-6-8 
		% ---
			\label{MOSES-6-8}

			Now this prophecy Adam spake, as he was moved upon by the Holy Ghost, and a genealogy was kept of the children of God. And this was the book of the generations of Adam, saying: In the day that God created man, in the likeness of God made he him;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:9} % *MOSES-6-9 
		% ---
			\label{MOSES-6-9}

			In the image of his own body, male and female, created he them, and blessed them, and called their name Adam, in the day when they were created and became living souls in the land upon the footstool of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:10} % *MOSES-6-10 
		% ---
			\label{MOSES-6-10}

			And Adam lived one hundred and thirty years, and begat a son in his own likeness, after his own image, and called his name Seth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:11} % *MOSES-6-11 
		% ---
			\label{MOSES-6-11}

			And the days of Adam, after he had begotten Seth, were eight hundred years, and he begat many sons and daughters;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:12} % *MOSES-6-12 
		% ---
			\label{MOSES-6-12}

			And all the days that Adam lived were nine hundred and thirty years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:13} % *MOSES-6-13 
		% ---
			\label{MOSES-6-13}

			Seth lived one hundred and five years, and begat Enos, and prophesied in all his days, and taught his son Enos in the ways of God; wherefore Enos prophesied also.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:14} % *MOSES-6-14 
		% ---
			\label{MOSES-6-14}

			And Seth lived, after he begat Enos, eight hundred and seven years, and begat many sons and daughters.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:15} % *MOSES-6-15 
		% ---
			\label{MOSES-6-15}

			And the children of men were numerous upon all the face of the land. And in those days Satan had great dominion among men, and raged in their hearts; and from thenceforth came wars and bloodshed; and a man's hand was against his own brother, in administering death, because of secret works, seeking for power.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:16} % *MOSES-6-16 
		% ---
			\label{MOSES-6-16}

			All the days of Seth were nine hundred and twelve years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:17} % *MOSES-6-17 
		% ---
			\label{MOSES-6-17}

			And Enos lived ninety years, and begat Cainan. And Enos and the residue of the people of God came out from the land, which was called Shulon, and dwelt in a land of promise, which he called after his own son, whom he had named Cainan.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:18} % *MOSES-6-18 
		% ---
			\label{MOSES-6-18}

			And Enos lived, after he begat Cainan, eight hundred and fifteen years, and begat many sons and daughters. And all the days of Enos were nine hundred and five years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:19} % *MOSES-6-19 
		% ---
			\label{MOSES-6-19}

			And Cainan lived seventy years, and begat Mahalaleel; and Cainan lived after he begat Mahalaleel eight hundred and forty years, and begat sons and daughters. And all the days of Cainan were nine hundred and ten years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:20} % *MOSES-6-20 
		% ---
			\label{MOSES-6-20}

			And Mahalaleel lived sixty-five years, and begat Jared; and Mahalaleel lived, after he begat Jared, eight hundred and thirty years, and begat sons and daughters. And all the days of Mahalaleel were eight hundred and ninety-five years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:21} % *MOSES-6-21 
		% ---
			\label{MOSES-6-21}

			And Jared lived one hundred and sixty-two years, and begat Enoch; and Jared lived, after he begat Enoch, eight hundred years, and begat sons and daughters. And Jared taught Enoch in all the ways of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:22} % *MOSES-6-22 
		% ---
			\label{MOSES-6-22}

			And this is the genealogy of the sons of Adam, who was the son of God, with whom God, himself, conversed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:23} % *MOSES-6-23 
		% ---
			\label{MOSES-6-23}

			And they were preachers of righteousness, and spake and prophesied, and called upon all men, everywhere, to repent; and faith was taught unto the children of men.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:24} % *MOSES-6-24 
		% ---
			\label{MOSES-6-24}

			And it came to pass that all the days of Jared were nine hundred and sixty-two years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:25} % *MOSES-6-25 
		% ---
			\label{MOSES-6-25}

			And Enoch lived sixty-five years, and begat Methuselah.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:26} % *MOSES-6-26 
		% ---
			\label{MOSES-6-26}

			And it came to pass that Enoch journeyed in the land, among the people; and as he journeyed, the Spirit of God descended out of heaven, and abode upon him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:27} % *MOSES-6-27 
		% ---
			\label{MOSES-6-27}

			And he heard a voice from heaven, saying: Enoch, my son, prophesy unto this people, and say unto them---Repent, for thus saith the Lord: I am angry with this people, and my fierce anger is kindled against them; for their hearts have waxed hard, and their ears are dull of hearing, and their eyes cannot see afar off;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:28} % *MOSES-6-28 
		% ---
			\label{MOSES-6-28}

			And for these many generations, ever since the day that I created them, have they gone astray, and have denied me, and have sought their own counsels in the dark; and in their own abominations have they devised murder, and have not kept the commandments, which I gave unto their father, Adam.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:29} % *MOSES-6-29 
		% ---
			\label{MOSES-6-29}

			Wherefore, they have foresworn themselves, and, by their oaths, they have brought upon themselves death; and a hell I have prepared for them, if they repent not;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:30} % *MOSES-6-30 
		% ---
			\label{MOSES-6-30}

			And this is a decree, which I have sent forth in the beginning of the world, from my own mouth, from the foundation thereof, and by the mouths of my servants, thy fathers, have I decreed it, even as it shall be sent forth in the world, unto the ends thereof.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:31} % *MOSES-6-31 
		% ---
			\label{MOSES-6-31}

			And when Enoch had heard these words, he bowed himself to the earth, before the Lord, and spake before the Lord, saying: Why is it that I have found favor in thy sight, and am but a lad, and all the people hate me; for I am slow of speech; wherefore am I thy servant?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:32} % *MOSES-6-32 
		% ---
			\label{MOSES-6-32}

			And the Lord said unto Enoch: Go forth and do as I have commanded thee, and no man shall pierce thee. Open thy mouth, and it shall be filled, and I will give thee utterance, for all flesh is in my hands, and I will do as seemeth me good.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:33} % *MOSES-6-33 
		% ---
			\label{MOSES-6-33}

			Say unto this people: Choose ye this day, to serve the Lord God who made you.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:34} % *MOSES-6-34 
		% ---
			\label{MOSES-6-34}

			Behold my Spirit is upon you, wherefore all thy words will I justify; and the mountains shall flee before you, and the rivers shall turn from their course; and thou shalt abide in me, and I in you; therefore walk with me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:35} % *MOSES-6-35 
		% ---
			\label{MOSES-6-35}

			And the Lord spake unto Enoch, and said unto him: Anoint thine eyes with clay, and wash them, and thou shalt see. And he did so.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:36} % *MOSES-6-36 
		% ---
			\label{MOSES-6-36}

			And he beheld the spirits that God had created; and he beheld also things which were not visible to the natural eye; and from thenceforth came the saying abroad in the land: A seer hath the Lord raised up unto his people.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:37} % *MOSES-6-37 
		% ---
			\label{MOSES-6-37}

			And it came to pass that Enoch went forth in the land, among the people, standing upon the hills and the high places, and cried with a loud voice, testifying against their works; and all men were offended because of him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:38} % *MOSES-6-38 
		% ---
			\label{MOSES-6-38}

			And they came forth to hear him, upon the high places, saying unto the tent-keepers: Tarry ye here and keep the tents, while we go yonder to behold the seer, for he prophesieth, and there is a strange thing in the land; a wild man hath come among us.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:39} % *MOSES-6-39 
		% ---
			\label{MOSES-6-39}

			And it came to pass when they heard him, no man laid hands on him; for fear came on all them that heard him; for he walked with God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:40} % *MOSES-6-40 
		% ---
			\label{MOSES-6-40}

			And there came a man unto him, whose name was Mahijah, and said unto him: Tell us plainly who thou art, and from whence thou comest?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:41} % *MOSES-6-41 
		% ---
			\label{MOSES-6-41}

			And he said unto them: I came out from the land of Cainan, the land of my fathers, a land of righteousness unto this day. And my father taught me in all the ways of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:42} % *MOSES-6-42 
		% ---
			\label{MOSES-6-42}

			And it came to pass, as I journeyed from the land of Cainan, by the sea east, I beheld a vision; and lo, the heavens I saw, and the Lord spake with me, and gave me commandment; wherefore, for this cause, to keep the commandment, I speak forth these words.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:43} % *MOSES-6-43 
		% ---
			\label{MOSES-6-43}

			And Enoch continued his speech, saying: The Lord which spake with me, the same is the God of heaven, and he is my God, and your God, and ye are my brethren, and why counsel ye yourselves, and deny the God of heaven?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:44} % *MOSES-6-44 
		% ---
			\label{MOSES-6-44}

			The heavens he made; the earth is his footstool; and the foundation thereof is his. Behold, he laid it, an host of men hath he brought in upon the face thereof.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:45} % *MOSES-6-45 
		% ---
			\label{MOSES-6-45}

			And death hath come upon our fathers; nevertheless we know them, and cannot deny, and even the first of all we know, even Adam.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:46} % *MOSES-6-46 
		% ---
			\label{MOSES-6-46}

			For a book of remembrance we have written among us, according to the pattern given by the finger of God; and it is given in our own language.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:47} % *MOSES-6-47 
		% ---
			\label{MOSES-6-47}

			And as Enoch spake forth the words of God, the people trembled, and could not stand in his presence.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:48} % *MOSES-6-48 
		% ---
			\label{MOSES-6-48}

			And he said unto them: Because that Adam fell, we are; and by his fall came death; and we are made partakers of misery and woe.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:49} % *MOSES-6-49 
		% ---
			\label{MOSES-6-49}

			Behold Satan hath come among the children of men, and tempteth them to worship him; and men have become carnal, sensual, and devilish, and are shut out from the presence of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:50} % *MOSES-6-50 
		% ---
			\label{MOSES-6-50}

			But God hath made known unto our fathers that all men must repent.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:51} % *MOSES-6-51 
		% ---
			\label{MOSES-6-51}

			And he called upon our father Adam by his own voice,%
				\footnote{%
					% \label{}%
					Compare \hyperlink{JST-OTR1-GENESIS-14-d}{JST OTR1 of Genesis 14}, part of which says that the order of the priesthood ``was delivered unto men by the calling of his [God's] own voice.'' 
					}
			saying: I am God; I made the world, and men before they were in the flesh.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:52} % *MOSES-6-52 
		% ---
			\label{MOSES-6-52}

			And he also said unto him: If thou wilt turn unto me, and hearken%
				\footnote{%
					% \label{}%
					Here between ``hearken'' and ``unto'' OT1-14 begins. At the top of this page John Whitmer wrote and partially circled the heeading, ``The Plan of Salvation.''
					}
			unto my voice, and believe, and repent of all thy transgressions, and be baptized, even in water, in the name of mine Only Begotten Son, who is full of grace and truth, which is Jesus Christ, the only name which shall be given under heaven, whereby salvation shall come unto the children of men, ye shall receive the gift of the Holy Ghost, asking all things in his name, and whatsoever ye shall ask, it shall be given you.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:53} % *MOSES-6-53 
		% ---
			\label{MOSES-6-53}

			And our father Adam spake unto the Lord, and said: Why is it that men must repent and be baptized in water? And the Lord said unto Adam: Behold I have forgiven thee thy transgression in the Garden of Eden.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:54} % *MOSES-6-54 
		% ---
			\label{MOSES-6-54}

			Hence came the saying abroad among the people, that the Son of God hath atoned for original guilt, wherein the sins of the parents cannot be answered upon the heads of the children, for they are whole from the foundation of the world.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:55} % *MOSES-6-55 
		% ---
			\label{MOSES-6-55}
			
			And the Lord spake unto Adam, saying: Inasmuch as thy children are conceived in sin,%
				\footnote{%
					% \label{MOSES-6-55-NOTE-conceived}%
					See \hyperref[JOHN-9-34]{John 9:34}, \gr{Ἐν ἁμαρτίαις σὺ ἐγεννήθης ὅλος}.
					}
			even so when they begin to grow up, sin conceiveth in their hearts, and they taste the bitter, that they may know to prize the good.%
				\footnote{%
					% \label{}%
					See \hyperref[DC29-39]{DC 29:39} and notes there.
					}

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:56} % *MOSES-6-56 
		% ---
			\label{MOSES-6-56}

			And it is given unto them to know good from evil; wherefore they are agents unto themselves, and I have given unto you another law and commandment.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:57} % *MOSES-6-57 
		% ---
			\label{MOSES-6-57}

			Wherefore teach it unto your children, that all men, everywhere, must repent, or they can in nowise inherit the kingdom of God, for no unclean thing can dwell there, or dwell in his presence; for, in the language of Adam, Man of Holiness is his name, and the name of his Only Begotten is the Son of Man, even Jesus Christ, a righteous Judge, who shall come in the meridian of time.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:58} % *MOSES-6-58 
		% ---
			\label{MOSES-6-58}

			Therefore I give unto you a commandment, to teach these things freely unto your children, saying:

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:59} % *MOSES-6-59 
		% ---
			\label{MOSES-6-59}

			That by reason of transgression cometh the fall, which fall bringeth death, and inasmuch as ye were born into the world by water, and blood, and the spirit, which I have made, and so became of dust a living soul, even so ye must be born again into the kingdom of heaven, of water, and of the Spirit, and be cleansed by blood, even the blood of mine Only Begotten; that ye might be sanctified from all sin, and enjoy the words of eternal life in this world, and eternal life in the world to come, even immortal glory;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
			
		% ---
		\paragraph{Moses 6:60} % *MOSES-6-60 
		% ---
			\label{MOSES-6-60}

			For by the water ye keep the commandment; by the Spirit ye are justified, and by the blood ye are sanctified;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:61} % *MOSES-6-61 
		% ---
			\label{MOSES-6-61}

			Therefore it is given to abide in you; the record of heaven; the Comforter; the peaceable things of immortal glory; the truth of all things; that which quickeneth all things, which maketh alive all things; that which knoweth all things, and hath all power according to wisdom, mercy, truth, justice, and judgment.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:62} % *MOSES-6-62 
		% ---
			\label{MOSES-6-62}

			And now, behold, I say unto you: This is the plan of salvation unto all men, through the blood of mine Only Begotten, who shall come in the meridian of time.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:63} % *MOSES-6-63 
		% ---
			\label{MOSES-6-63}

			And behold, all things have their likeness, and all things are created and made to bear record of me, both things which are temporal, and things which are spiritual; things which are in the heavens above, and things which are on the earth, and things which are in the earth, and things which are under the earth, both above and beneath: all things bear record of me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:64} % *MOSES-6-64 
		% ---
			\label{MOSES-6-64}

			And it came to pass, when the Lord had spoken with Adam, our father, that Adam cried unto the Lord, and he was caught away by the Spirit of the Lord, and was carried down into the water, and was laid under the water, and was brought forth out of the water.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:65} % *MOSES-6-65 
		% ---
			\label{MOSES-6-65}

			And thus he was baptized, and the Spirit of God descended upon him, and thus he was born of the Spirit, and became quickened in the inner man.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:66} % *MOSES-6-66 
		% ---
			\label{MOSES-6-66}

			And he heard a voice out of heaven, saying: Thou art baptized with fire, and with the Holy Ghost. This is the record of the Father, and the Son, from henceforth and forever;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:67} % *MOSES-6-67 
		% ---
			\label{MOSES-6-67}

			And thou art after the order of him who was without beginning of days or end of years, from all eternity to all eternity.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 6:68} % *MOSES-6-68 
		% ---
			\label{MOSES-6-68}

			Behold, thou art one in me,%
				\footnote{%
					% \label{}%
					For ``one in me,'' see \hyperref[DC35-2]{DC 35:2}.
					}
			a son of God; and thus may all become my sons. Amen.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 7} % *MOSES-7
	% -----
		\label{MOSES-7}
		
		% ---
		\paragraph{Moses 7:1} % *MOSES-7-1 
		% ---
			\label{MOSES-7-1}

				\footnote{%
					% \label{}%
					Compare \hyperref[DC38]{DC 38} as a companion piece to this entire chapter.
					}%
			AND it came to pass that Enoch continued his speech, saying: Behold, our father Adam taught these things, and many have believed and become the sons of God, and many have believed not, and have perished in their sins, and are looking forth with fear, in torment, for the fiery indignation of the wrath of God to be poured out upon them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:2} % *MOSES-7-2 
		% ---
			\label{MOSES-7-2}

			And from that time forth Enoch began to prophesy, saying unto the people, that: As I was journeying, and stood upon the place Mahujah, and cried unto the Lord, there came a voice out of heaven, saying---Turn ye, and get ye upon the mount Simeon.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:3} % *MOSES-7-3 
		% ---
			\label{MOSES-7-3}

			And it came to pass that I turned and went up on the mount; and as I stood upon the mount, I beheld the heavens open, and I was clothed upon with glory;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:4} % *MOSES-7-4 
		% ---
			\label{MOSES-7-4}

			And I saw the Lord; and he stood before my face, and he talked with me, even as a man talketh one with another, face to face; and he said unto me: Look, and I will show unto thee the world for the space of many generations.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:5} % *MOSES-7-5 
		% ---
			\label{MOSES-7-5}

			And it came to pass that I beheld in the valley of Shum, and lo, a great people which dwelt in tents, which were the people of Shum.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:6} % *MOSES-7-6 
		% ---
			\label{MOSES-7-6}

			And again the Lord said unto me: Look; and I looked towards the north, and I beheld the people of Canaan, which dwelt in tents.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:7} % *MOSES-7-7 
		% ---
			\label{MOSES-7-7}

			And the Lord said unto me: Prophesy; and I prophesied, saying: Behold the people of Canaan, which are numerous, shall go forth in battle array against the people of Shum, and shall slay them that they shall utterly be destroyed; and the people of Canaan shall divide themselves in the land, and the land shall be barren and unfruitful, and none other people shall dwell there but the people of Canaan;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:8} % *MOSES-7-8 
		% ---
			\label{MOSES-7-8}

			For behold, the Lord shall curse the land with much heat, and the barrenness thereof shall go forth forever; and there was a blackness came upon all the children of Canaan, that they were despised among all people.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:9} % *MOSES-7-9 
		% ---
			\label{MOSES-7-9}

			And it came to pass that the Lord said unto me: Look; and I looked, and I beheld the land of Sharon, and the land of Enoch, and the land of Omner, and the land of Heni, and the land of Shem, and the land of Haner, and the land of Hanannihah, and all the inhabitants thereof;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:10} % *MOSES-7-10 
		% ---
			\label{MOSES-7-10}

			And the Lord said unto me: Go to this people, and say unto them---Repent, lest I come out and smite them with a curse, and they die.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:11} % *MOSES-7-11 
		% ---
			\label{MOSES-7-11}

			And he gave unto me a commandment that I should baptize in the name of the Father, and of the Son, which is full of grace and truth, and of the Holy Ghost, which beareth record of the Father and the Son.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:12} % *MOSES-7-12 
		% ---
			\label{MOSES-7-12}

			And it came to pass that Enoch continued to call upon all the people, save it were the people of Canaan, to repent;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:13} % *MOSES-7-13 
		% ---
			\label{MOSES-7-13}

			And so great was the faith of Enoch that he led the people of God, and their enemies came to battle against them; and he spake the word of the Lord, and the earth trembled, and the mountains fled, even according to his command; and the rivers of water were turned out of their course; and the roar of the lions was heard out of the wilderness; and all nations feared greatly, so powerful was the word of Enoch, and so great was the power of the language which God had given him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:14} % *MOSES-7-14 
		% ---
			\label{MOSES-7-14}

			There also came up a land out of the depth of the sea, and so great was the fear of the enemies of the people of God, that they fled and stood afar off and went upon the land which came up out of the depth of the sea.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:15} % *MOSES-7-15 
		% ---
			\label{MOSES-7-15}

			And the giants of the land, also, stood afar off; and there went forth a curse upon all people that fought against God;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:16} % *MOSES-7-16 
		% ---
			\label{MOSES-7-16}

			And from that time forth there were wars and bloodshed among them; but the Lord came and dwelt with his people, and they dwelt in righteousness.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:17} % *MOSES-7-17 
		% ---
			\label{MOSES-7-17}

			The fear of the Lord was upon all nations, so great was the glory of the Lord, which was upon his people. And the Lord blessed the land, and they were blessed upon the mountains, and upon the high places, and did flourish.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:18} % *MOSES-7-18 
		% ---
			\label{MOSES-7-18}

			And the Lord called his people ZION, because they were of one heart and one mind,%
				\footnote{%
					% \label{}%
					Compare \hyperref[PHILIPPIANS-1-27]{Philippians 1:27}, \hyperref[PHILIPPIANS-2-2]{Philippians 2:2}, \hyperref[2NEPHI-1-21]{2 Nephi 1:21}, \hyperref[DC45-65]{DC 45:65}, \hyperref[DC97-21]{DC 97:21}, 
					}
			and dwelt in righteousness; and there was no poor among them.%
				\footnote{%
					% \label{}%
					See also \hyperref[CONSECRATION]{Consecration}; \hyperref[DC51-9]{DC 51:9} (and section 50 more generally); 
					}

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:19} % *MOSES-7-19 
		% ---
			\label{MOSES-7-19}

			And Enoch continued his preaching in righteousness unto the people of God. And it came to pass in his days, that he built a city that was called the City of Holiness, even ZION.%
				\footnote{%
					% \label{}%
					See \hyperref[DC45-11]{DC 45:11--14}.
					} 

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:20} % *MOSES-7-20 
		% ---
			\label{MOSES-7-20}

			And it came to pass that Enoch talked with the Lord; and he said unto the Lord: Surely Zion shall dwell in safety forever. But the Lord said unto Enoch: Zion have I blessed, but the residue of the people have I cursed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:21} % *MOSES-7-21 
		% ---
			\label{MOSES-7-21}

			And it came to pass that the Lord showed unto Enoch all the inhabitants of the earth; and he beheld, and lo, Zion, in process of time, was taken up into heaven. And the Lord said unto Enoch: Behold mine abode forever.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:22} % *MOSES-7-22 
		% ---
			\label{MOSES-7-22}

			And it came to pass that the Lord showed unto Enoch all the inhabitants of the earth; and he beheld, and lo, Zion, in process of time, was taken up into heaven. And the Lord said unto Enoch: Behold mine abode forever.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:23} % *MOSES-7-23 
		% ---
			\label{MOSES-7-23}

			And after that Zion was taken up into heaven, Enoch beheld, and lo, all the nations of the earth were before him;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:24} % *MOSES-7-24 
		% ---
			\label{MOSES-7-24}

			And there came generation upon generation; and Enoch was high and lifted up, even in the bosom of the Father, and of the Son of Man; and behold, the power of Satan was upon all the face of the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:25} % *MOSES-7-25 
		% ---
			\label{MOSES-7-25}

			And he saw angels descending out of heaven; and he heard a loud voice saying: Wo, wo be unto the inhabitants of the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:26} % *MOSES-7-26 
		% ---
			\label{MOSES-7-26}

			And he beheld Satan; and he had a great chain in his hand, and it veiled the whole face of the earth with darkness;	and he looked up and laughed, and his angels rejoiced.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:27} % *MOSES-7-27 
		% ---
			\label{MOSES-7-27}

			And Enoch beheld angels descending out of heaven, bearing testimony of the Father and Son; and the Holy Ghost fell on many, and they were caught up by the powers of heaven into Zion.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:28} % *MOSES-7-28 
		% ---
			\label{MOSES-7-28}

			And it came to pass that the God of heaven looked upon the residue of the people, and he wept; and Enoch bore record of it, saying: How is it that the heavens weep, and shed forth their tears as the rain upon the mountains?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:29} % *MOSES-7-29 
		% ---
			\label{MOSES-7-29}

			And Enoch said unto the Lord: How is it that thou canst weep, seeing thou art holy, and from all eternity to all eternity?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:30} % *MOSES-7-30 
		% ---
			\label{MOSES-7-30}

			And were it possible that man could number the particles of the earth, yea, millions of earths like this, it would not be a beginning to the number of thy creations; and thy curtains are stretched out still; and yet thou art there, and thy bosom is there; and also thou art just; thou art merciful and kind forever;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:31} % *MOSES-7-31 
		% ---
			\label{MOSES-7-31}

			And thou hast taken Zion to thine own bosom, from all thy creations, from all eternity to all eternity; and naught but peace, justice, and truth is the habitation of thy throne; and mercy shall go before thy face and have no end; how is it thou canst weep?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:32} % *MOSES-7-32 
		% ---
			\label{MOSES-7-32}

			The Lord said unto Enoch: Behold these thy brethren; they are the workmanship of mine own hands, and I gave unto them their knowledge, in the day I created them; and in the Garden of Eden, gave I unto man his agency;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:33} % *MOSES-7-33 
		% ---
			\label{MOSES-7-33}

			And unto thy brethren have I said, and also given commandment, that they should love one another, and that they should choose me, their Father; but behold, they are without affection, and they hate their own blood;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:34} % *MOSES-7-34 
		% ---
			\label{MOSES-7-34}

			And the fire of mine indignation is kindled against them; and in my hot displeasure will I send in the floods upon them, for my fierce anger is kindled against them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:35} % *MOSES-7-35 
		% ---
			\label{MOSES-7-35}

			Behold, I am God; Man of Holiness is my name; Man of Counsel is my name; and Endless and Eternal is my name, also.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:36} % *MOSES-7-36 
		% ---
			\label{MOSES-7-36}

			Wherefore, I can stretch forth mine hands and hold all the creations which I have made; and mine eye can pierce them also, and among all the workmanship of mine hands there has not been so great wickedness as among thy brethren.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:37} % *MOSES-7-37 
		% ---
			\label{MOSES-7-37}

			But behold, their sins shall be upon the heads of their fathers; Satan shall be their father, and misery shall be their doom; and the whole heavens shall weep over them, even all the workmanship of mine hands; wherefore should not the heavens weep, seeing these shall suffer?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:38} % *MOSES-7-38 
		% ---
			\label{MOSES-7-38}

			But behold, these which thine eyes are upon shall perish in the floods; and behold, I will shut them up; a prison have I prepared for them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:39} % *MOSES-7-39 
		% ---
			\label{MOSES-7-39}

			And That which I have chosen hath pled before my face. Wherefore, he suffereth for their sins; inasmuch as they will repent in the day that my Chosen shall return unto me, and until that day they shall be in torment;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:40} % *MOSES-7-40 
		% ---
			\label{MOSES-7-40}

			Wherefore, for this shall the heavens weep, yea, and all the workmanship of mine hands.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:41} % *MOSES-7-41 
		% ---
			\label{MOSES-7-41}

			And it came to pass that the Lord spake unto Enoch, and told Enoch all the doings of the children of men; wherefore Enoch knew, and looked upon their wickedness, and their misery, and wept and stretched forth his arms, and his heart swelled wide as eternity;%
				\footnote{%
					% \label{}%
					In a \hyperref[EMS-1840-JS-14-MAY-1840]{14 May 1840} letter, JS wrote the following to Orson Hyde and John E. Page: ``therefore, inasmuch as you are to be instrumental in this great work, he will endow you with power, wisdom, might, and inteligence; and every qualification necessary; while your minds will expand wider and wider, untill you can circumscribe the Earth, \& the Heavens, and reach forth into eternity; contemplate the mighty acts of Jehovah, in all their variety \& glory.''
					}
			and his bowels yearned; and all eternity shook.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:42} % *MOSES-7-42 
		% ---
			\label{MOSES-7-42}

			And Enoch also saw Noah, and his family; that the posterity of all the sons of Noah should be saved with a temporal salvation;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:43} % *MOSES-7-43 
		% ---
			\label{MOSES-7-43}

			Wherefore Enoch saw that Noah built an ark; and that the Lord smiled upon it, and held it in his own hand; but upon the residue of the wicked the floods came and swallowed them up.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:44} % *MOSES-7-44 
		% ---
			\label{MOSES-7-44}

			And as Enoch saw this, he had bitterness of soul, and wept over his brethren, and said unto the heavens: I will refuse to be comforted; but the Lord said unto Enoch: Lift up your heart, and be glad; and look.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:45} % *MOSES-7-45 
		% ---
			\label{MOSES-7-45}

			And it came to pass that Enoch looked; and from Noah, he beheld all the families of the earth; and he cried unto the Lord, saying: When shall the day of the Lord come? When shall the blood of the Righteous be shed, that all they that mourn may be sanctified and have eternal life?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:46} % *MOSES-7-46 
		% ---
			\label{MOSES-7-46}

			And the Lord said: It shall be in the meridian of time, in the days of wickedness and vengeance.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:47} % *MOSES-7-47 
		% ---
			\label{MOSES-7-47}

			And behold, Enoch saw the day of the coming of the Son of Man, even in the flesh; and his soul rejoiced, saying: The Righteous is lifted up, and the Lamb is slain from the foundation of the world; and through faith I am in the bosom of the Father, and behold, Zion is with me.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:48} % *MOSES-7-48 
		% ---
			\label{MOSES-7-48}

			And it came to pass that Enoch looked upon the earth; and he heard a voice from the bowels thereof, saying: Wo, wo is me, the mother of men; I am pained, I am weary, because of the wickedness of my children. When shall I rest, and be cleansed from the filthiness which is gone forth out of me? When will my Creator sanctify me, that I may rest, and righteousness for a season abide upon my face?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:49} % *MOSES-7-49 
		% ---
			\label{MOSES-7-49}

			And when Enoch heard the earth mourn, he wept, and cried unto the Lord, saying: O Lord, wilt thou not have compassion upon the earth? Wilt thou not bless the children of Noah?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:50} % *MOSES-7-50 
		% ---
			\label{MOSES-7-50}

			And it came to pass that Enoch continued his cry unto the Lord, saying: I ask thee, O Lord, in the name of thine Only Begotten, even Jesus Christ, that thou wilt have mercy upon Noah and his seed, that the earth might never more be covered by the floods.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:51} % *MOSES-7-51 
		% ---
			\label{MOSES-7-51}

			And the Lord could not withhold; and he covenanted with Enoch, and sware unto him with an oath, that he would stay the floods; that he would call upon the children of Noah;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:52} % *MOSES-7-52 
		% ---
			\label{MOSES-7-52}

			And he sent forth an unalterable decree, that a remnant of his seed should always be found among all nations, while the earth should stand;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:53} % *MOSES-7-53 
		% ---
			\label{MOSES-7-53}

			And the Lord said: Blessed is he through whose seed Messiah shall come; for he saith---I am Messiah, the King of Zion, the Rock of Heaven, which is broad as eternity; whoso cometh in at the gate and climbeth up by me shall never fall; wherefore, blessed are they of whom I have spoken, for they shall come forth with songs of everlasting joy.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:54} % *MOSES-7-54 
		% ---
			\label{MOSES-7-54}

			And it came to pass that Enoch cried unto the Lord, saying: When the Son of Man cometh in the flesh, shall the earth rest? I pray thee, show me these things.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:55} % *MOSES-7-55 
		% ---
			\label{MOSES-7-55}

			And the Lord said unto Enoch: Look, and he looked and beheld the Son of Man lifted up on the cross, after the manner of men;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:56} % *MOSES-7-56 
		% ---
			\label{MOSES-7-56}

			And he heard a loud voice; and the heavens were veiled; and all the creations of God mourned; and the earth groaned;%
				\footnote{%
					% \label{MOSES-7-56-groaned}%
					See \hyperref[ROMANS-8-22]{Romans 8:22}; \hyperref[DC84-49]{DC 84:49--53}; 
					}
			and the rocks were rent; and the saints arose, and were crowned at the right hand of the Son of Man, with crowns of glory;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:57} % *MOSES-7-57 
		% ---
			\label{MOSES-7-57}

			And as many of the spirits as were in prison came forth, and stood on the right hand of God; and the remainder were reserved in chains of darkness until the judgment of the great day.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:58} % *MOSES-7-58 
		% ---
			\label{MOSES-7-58}

			And again Enoch wept and cried unto the Lord, saying: When shall the earth rest?

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:59} % *MOSES-7-59 
		% ---
			\label{MOSES-7-59}

			And Enoch beheld the Son of Man ascend up unto the Father; and he called unto the Lord, saying: Wilt thou not come again upon the earth? Forasmuch as thou art God, and I know thee, and thou hast sworn unto me, and commanded me that I should ask in the name of thine Only Begotten; thou hast made me, and given unto me a right to thy throne, and not of myself, but through thine own grace; wherefore, I ask thee if thou wilt not come again on the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:60} % *MOSES-7-60 
		% ---
			\label{MOSES-7-60}

			And the Lord said unto Enoch: As I live, even so will I come in the last days, in the days of wickedness and vengeance, to fulfil the oath which I have made unto you concerning the children of Noah;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:61} % *MOSES-7-61 
		% ---
			\label{MOSES-7-61}

			And the day shall come that the earth shall rest, but before that day the heavens shall be darkened, and a veil of darkness shall cover the earth; and the heavens shall shake, and also the earth; and great tribulations shall be among the children of men, but my people will I preserve;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:62} % *MOSES-7-62 
		% ---
			\label{MOSES-7-62}

			And righteousness will I send down out of heaven; and truth will I send forth out of the earth, to bear testimony of mine Only Begotten; his resurrection from the dead; yea, and also the resurrection of all men; and righteousness and truth will I cause to sweep the earth as with a flood, to gather out mine elect from the four quarters of the earth, unto a place which I shall prepare, an Holy City, that my people may gird up their loins, and be looking forth for the time of my coming; for there shall be my tabernacle, and it shall be called Zion, a New Jerusalem.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:63} % *MOSES-7-63 
		% ---
			\label{MOSES-7-63}

			And the Lord said unto Enoch: Then shalt thou and all thy city meet them there, and we will receive them into our bosom, and they shall see us; and we will fall upon their necks, and they shall fall upon our necks, and we will kiss each other;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:64} % *MOSES-7-64 
		% ---
			\label{MOSES-7-64}

			And there shall be mine abode, and it shall be Zion, which shall come forth out of all the creations which I have made; and for the space of a thousand years the earth shall rest.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:65} % *MOSES-7-65 
		% ---
			\label{MOSES-7-65}

			And it came to pass that Enoch saw the day of the coming of the Son of Man, in the last days, to dwell on the earth in righteousness for the space of a thousand years;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:66} % *MOSES-7-66 
		% ---
			\label{MOSES-7-66}

			But before that day he saw great tribulations among the wicked; and he also saw the sea, that it was troubled, and men's hearts failing them, looking forth with fear for the judgments of the Almighty God, which should come upon the wicked.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:67} % *MOSES-7-67 
		% ---
			\label{MOSES-7-67}

			And the Lord showed Enoch all things, even unto the end of the world; and he saw the day of the righteous, the hour of their redemption, and received a fulness of joy;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:68} % *MOSES-7-68 
		% ---
			\label{MOSES-7-68}

			And all the days of Zion, in the days of Enoch, were three hundred and sixty-five years.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 7:69} % *MOSES-7-69 
		% ---
			\label{MOSES-7-69}

			And Enoch and all his people walked with God, and he dwelt in the midst of Zion; and it came to pass that Zion was not, for God received it up into his own bosom;%
			\footnote{%
				% \label{}%
				Compare \hyperref[DC38-4]{DC 38:4}.
				} 
			and from thence went forth the saying, ZION IS FLED.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}
		
	% -----
	\subsubsection{Moses 8} % *MOSES-8
	% -----
		\label{MOSES-8}
		
		% ---
		\paragraph{Moses 8:1} % *MOSES-8-1 
		% ---
			\label{MOSES-8-1}

			AND all the days of Enoch were four hundred and thirty years.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:2} % *MOSES-8-2 
		% ---
			\label{MOSES-8-2}

			And it came to pass that Methuselah, the son of Enoch, was not taken, that the covenants of the Lord might be fulfilled, which he made to Enoch; for he truly covenanted with Enoch that Noah should be of the fruit of his loins.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:3} % *MOSES-8-3 
		% ---
			\label{MOSES-8-3}

			And it came to pass that Methuselah prophesied that from his loins should spring all the kingdoms of the earth (through Noah), and he took glory unto himself.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:4} % *MOSES-8-4 
		% ---
			\label{MOSES-8-4}

			And there came forth a great famine into the land, and the Lord cursed the earth with a sore curse, and many of the inhabitants thereof died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:5} % *MOSES-8-5 
		% ---
			\label{MOSES-8-5}

			And it came to pass that Methuselah lived one hundred and eighty-seven years, and begat Lamech;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:6} % *MOSES-8-6 
		% ---
			\label{MOSES-8-6}

			And Methuselah lived, after he begat Lamech, seven hundred and eighty-two years, and begat sons and daughters;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:7} % *MOSES-8-7 
		% ---
			\label{MOSES-8-7}

			And all the days of Methuselah were nine hundred and sixty-nine years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:8} % *MOSES-8-8 
		% ---
			\label{MOSES-8-8}

			And Lamech lived one hundred and eighty-two years, and begat a son,

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:9} % *MOSES-8-9 
		% ---
			\label{MOSES-8-9}

			And he called his name Noah, saying: This son shall comfort us concerning our work and toil of our hands, because of the ground which the Lord hath cursed.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:10} % *MOSES-8-10 
		% ---
			\label{MOSES-8-10}

			And Lamech lived, after he begat Noah, five hundred and ninety-five years, and begat sons and daughters;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:11} % *MOSES-8-11 
		% ---
			\label{MOSES-8-11}

			And all the days of Lamech were seven hundred and seventy-seven years, and he died.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:12} % *MOSES-8-12 
		% ---
			\label{MOSES-8-12}

			And Noah was four hundred and fifty years old, and begat Japheth; and forty-two years afterward he begat Shem of her who was the mother of Japheth, and when he was five hundred years old he begat Ham.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:13} % *MOSES-8-13 
		% ---
			\label{MOSES-8-13}

			And Noah and his sons hearkened unto the Lord, and gave heed, and they were called the sons of God.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:14} % *MOSES-8-14 
		% ---
			\label{MOSES-8-14}

			And when these men began to multiply on the face of the earth, and daughters were born unto them, the sons of men saw that those daughters were fair, and they took them wives, even as they chose.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:15} % *MOSES-8-15 
		% ---
			\label{MOSES-8-15}

			And the Lord said unto Noah: The daughters of thy sons have sold themselves; for behold mine anger is kindled against the sons of men, for they will not hearken to my voice.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:16} % *MOSES-8-16 
		% ---
			\label{MOSES-8-16}

			And it came to pass that Noah prophesied, and taught the things of God, even as it was in the beginning.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:17} % *MOSES-8-17 
		% ---
			\label{MOSES-8-17}

			And the Lord said unto Noah: My Spirit shall not always strive with man, for he shall know that all flesh shall die; yet his days shall be an hundred and twenty years; and if men do not repent, I will send in the floods upon them.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:18} % *MOSES-8-18 
		% ---
			\label{MOSES-8-18}

			And in those days there were giants on the earth, and they sought Noah to take away his life; but the Lord was with Noah, and the power of the Lord was upon him.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:19} % *MOSES-8-19 
		% ---
			\label{MOSES-8-19}

			And the Lord ordained Noah after his own order, and commanded him that he should go forth and declare his Gospel unto the children of men, even as it was given unto Enoch.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:20} % *MOSES-8-20 
		% ---
			\label{MOSES-8-20}

			And it came to pass that Noah called upon the children of men that they should repent; but they hearkened not unto his words;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:21} % *MOSES-8-21 
		% ---
			\label{MOSES-8-21}

			And also, after that they had heard him, they came up before him, saying: Behold, we are the sons of God; have we not taken unto ourselves the daughters of men? And are we not eating and drinking, and marrying and giving in marriage? And our wives bear unto us children, and the same are mighty men, which are like unto men of old, men of great renown. And they hearkened not unto the words of Noah.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:22} % *MOSES-8-22 
		% ---
			\label{MOSES-8-22}

			And God saw that the wickedness of men had become great in the earth; and every man was lifted up in the imagination of the thoughts of his heart, being only evil continually.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:23} % *MOSES-8-23 
		% ---
			\label{MOSES-8-23}

			And it came to pass that Noah continued his preaching unto the people, saying: Hearken, and give heed unto my words;

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:24} % *MOSES-8-24 
		% ---
			\label{MOSES-8-24}

			Believe and repent of your sins and be baptized in the name of Jesus Christ, the Son of God, even as our fathers, and ye shall receive the Holy Ghost, that ye may have all things made manifest; and if ye do not this, the floods will come in upon you; nevertheless they hearkened not.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:25} % *MOSES-8-25 
		% ---
			\label{MOSES-8-25}

			And it repented Noah, and his heart was pained that the Lord had made man on the earth, and it grieved him at the heart.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:26} % *MOSES-8-26 
		% ---
			\label{MOSES-8-26}

			And the Lord said: I will destroy man whom I have created, from the face of the earth, both man and beast, and the creeping things, and the fowls of the air; for it repenteth Noah that I have created them, and that I have made them; and he hath called upon me; for they have sought his life.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:27} % *MOSES-8-27 
		% ---
			\label{MOSES-8-27}

			And thus Noah found grace in the eyes of the Lord; for Noah was a just man, and perfect in his generation; and he walked with God, as did also his three sons, Shem, Ham, and Japheth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:28} % *MOSES-8-28 
		% ---
			\label{MOSES-8-28}

			The earth was corrupt before God, and it was filled with violence.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:29} % *MOSES-8-29 
		% ---
			\label{MOSES-8-29}

			And God looked upon the earth, and, behold, it was corrupt, for all flesh had corrupted its way upon the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}

		% ---
		\paragraph{Moses 8:30} % *MOSES-8-30 
		% ---
			\label{MOSES-8-30}

			And God said unto Noah: The end of all flesh is come before me, for the earth is filled with violence, and behold I will destroy all flesh from off the earth.

			% \begin{framed}
			% \scriptsize
				% \begin{compactdesc}
					% \item[OTR1] 
					% \item[OTRJW]
					% \item[OTR2] 
				% \end{compactdesc}
			% \end{framed}